...
for (var i = 5;i<message.content.length;i++){
    if (sel==0 && "qwertyuiopasdfghjklzxcvbn".indexOf(message.content.charAt(i))>=0){
        quer+=message.content.charAt(i);
    } else if (message.content.charAt(i)=='|'){
        newcap=message.content.slice(i+1);
        break
    }
}
        ...
        for (var i2 = 0; i2 < ii.length - 2; i2++) {
            if (ii.charAt(i2) === ' ') {
                for (var j = i2 + 2; j < ii.length; j++) {
                    if (ii.charAt(j) === ' ') {
                        lc = levenshtein(stripKeepSpaces(quer),ii.slice(i2+1,j)) - ((ii.length-j+i2-1)/(10*ii.length));
